...
class strFuncoes {
			...
			if( (!empty($p)) AND ($p != "\n") AND ($p != "") AND ($p != " ")){
				...
				if (ctype_upper($cmp)) {
					...
					if ($li){
						...
						$p = str_replace(strtolower($key), "<strong>".strtolower($key)."</strong>", $p);
	...
	public function RetiraPreposicao($palavras){
		...
		$vetStr = explode(" ", $palavras);
		$tam = count($vetStr);
		$strBase = "a,ante,até,após,de,desde,em,entre,com,contra,para,por,perante,sem,sob,sobre,na,no";
	
		$preposicoes = explode(',',$strBase);
		$vetStr = array_diff($vetStr, $preposicoes);
...
public function buscaVetorAprox($vetPalavra, $vetKey, $resultados=1, $buscaExata="") {

$vetFinal= array();
$vetEncontrados=array();
$vetEncontradosAprox=array();



$limit = $resultados;
$searchVet = $vetPalavra;


clearstatcache();

$contVetEncontrados=0;
$contVetEncontradosAprox=0;
$ix=0;
$val="";
$x=0;
$it=0;
...
$lenVetBuscaExata = sizeof($vetPalavra);

$lenVetBuscaExataPiece=$lenVetBuscaExata;

$lenVetSemPreposicao = sizeof(implode(" ", $this->RetiraPreposicao($buscaExata)));

	for ($xp=0;$xp<=($lenVetBuscaExata+($lenVetBuscaExata/2));$xp++){

		$val="";

		$lenVetBuscaExataPiece = sizeof($searchVet);
		$valKeyRestante = implode(" ", $searchVet);

		if ($xp>$lenVetBuscaExata){

			$sort = rand(0,$lenVetSemPreposicao);
			// $valKeyRestante = $vetPalavra[$sort];			

		}

		$valKeyRestante = strtolower($this->removeAcentos($valKeyRestante));

		foreach ($vetKey as $key => $val) {

			$valKeyBase = implode(" ", $this->RetiraPreposicao($val['key']));
			$valKeyBase = strtolower($this->removeAcentos($valKeyBase));


			if (stripos($valKeyBase, $valKeyRestante)===0)

			{
			
				$vetEncontrados[$it] = $key; 
				$it++;
				$contVetEncontrados++;
...

			{
				...
				$vetEncontradosAprox[$it2] = $key; 
				$it2++;
				$contVetEncontradosAprox++;

			}

		}

//Varredura Primeiro Nível

		foreach ($vetEncontrados as $keyFound => $valFound) {
				
			$contTriagem=0;
	
			foreach ($vetFinal as $keyFinal => $valFinal) {

				if($valFound==$valFinal['id']){
			...
			if(!$contTriagem){
				...
				$vetFinal[$ix]['id']=$valFound;
					
				$ix++;
				if ($ix==$limit) { return $vetFinal ; }

			} 

		}

		if ($lenVetBuscaExataPiece>4){ unset($searchVet[$lenVetBuscaExataPiece]);} else {unset($searchVet[$lenVetBuscaExataPiece-1]);}
	}


//Varredura Segundo Nível

foreach ($vetEncontradosAprox as $keyFound => $valFound) {
...
//Varredura Terceiro Nível

	if ($ix<$limit){
		
		$falta = $limit-sizeof($vetFinal);
...
public function getVetorbyId($id, $vetKey) {
